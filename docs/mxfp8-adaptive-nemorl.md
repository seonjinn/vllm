...
Use these exact source coordinates for the functional implementation:

```text
fork:        https://github.com/puririshi98/vllm.git
branch:      sna/mxfp8-adaptive-v0.20.2-nemorl
commit:      e32ce4fdd30ef313e10bf3a328352ead2a4c0054
base commit: 5246e3c5df5fb8266b50ceaa6eca2836fb2d13b1
vLLM:       0.20.2
...
```text
inventory -> shmoo -> promote -> validate
```

Use an initial bootstrap manifest with empty tactic tables and runner default
`-1`. Its compatibility must be exact for:
...
```bash
export VLLM_MXFP8_DENSE_CONFIG_FILE=/shared/qwen3_30ba3b_tp1_v0202_rollout_trace_bootstrap.json
...
python tools/mxfp8/offline_shmoo.py inventory \
  --bootstrap-manifest /shared/qwen3_30ba3b_tp1_v0202_rollout_trace_bootstrap.json \
  --trace /shared/qwen_trace/adaptive_dispatch_node0.jsonl \
  --trace /shared/qwen_trace/dense_shapes_node0.jsonl \
...
python tools/mxfp8/offline_shmoo.py shmoo \
  ...
  --container-sha256 <64-hex-container-sha256>
```
...
bypass this dense MXFP8 path. Inventory exits nonzero with
`zero eligible dense MXFP8 trace records` when there are no hits.
