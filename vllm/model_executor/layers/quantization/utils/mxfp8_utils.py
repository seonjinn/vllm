...
def _mxfp8_trtllm_linear_fixed_impl(
    ...
    *,
    ...
    if not torch.compiler.is_compiling() and (
        ...
        _, requested_tactic = _MXFP8_TACTIC_SOURCES.get(
            (id(state), key),
        ...
        if _MXFP8_TRTLLM_TRACE_CALLBACK is not None:
...
def _resolve_mxfp8_trtllm_call(
    x: torch.Tensor,
    weight: torch.Tensor,
    weight_scale: torch.Tensor,
    output_features: int,
...
def mxfp8_trtllm_adaptive_linear(
    ...
    tactic, tactic_source = _resolve_mxfp8_trtllm_call(
        x,
        weight,
        weight_scale,
        output_features,
    ...
    *,
    ...
    for node in graph.nodes:
        if node.op == "call_function" and node.target == marker_op:
            ...
            if tactic_override is not None:
                if len(node.args) < 6:
                    raise RuntimeError(
    ...
    def __call__(self, graph: torch.fx.Graph) -> None:
        ...
        replaced = _specialize_mxfp8_adaptive_layout_graph(
            ...
            tactic_override=(
                ...
                else (-1, "dynamic_compile_default")
            ),
        )
        if replaced == 0:
            return
